...
from spider import spider, cj, allowedExtensions, getAuthenticated , getAuthenticatedWithoutCSRF
...
class ConfHandler(ContentHandler):
	def __init__(self):
		...
		self.inAction  = False
		self.string = ""
		self.listActions = ["crystal", "sql","bsql","xss","include","backup","javascript", "session", "command"]
...
def investigate(url, what = "xss"):
	...
	if what == "xss":
		...
		localDB = database
	elif what == "commandInjection":
		from commandInjection import process
		localDB = database
	elif what == "session":
...
if __name__ == '__main__':
	...
	option_commandInjection = False
	option_authenticate = ""
	option_user = ""
	option_pwd = ""
	option_authWithCSRF = False
	...
	if len(sys.argv) > 1:
		...
		parser.add_option("-y", "--commandInjection", dest="commandInjection", action="store_true",default=False, help="command injection attack")
		parser.add_option("-a", "--auth", dest="authURL", help="authentication")
		parser.add_option("-n", "--user", dest="user", help="username")
		parser.add_option("-p", "--pwd", dest="pwd", help="password")
		parser.add_option("-w", "--authWithCSRF", dest="authWithCSRF",default=False, help="authWithCSRF authentication")
		...
		option_commandInjection = options.commandInjection
		option_auth = options.authURL
		option_user = options.user
		option_pwd = options.pwd
		option_authWithCSRF = option_authWithCSRF
	else:
		try:
		...
		option_commandInjection= "commandInjection" in confActions
	...
	if option_auth:
		...
		password = option_pwd
		if option_authWithCSRF:
			getAuthenticated(username,password,authURL)
		else:
			getAuthenticatedWithoutCSRF(username,password,authURL)
	...
	try:
		...
		if option_commandInjection:
			investigate(archives_url, "commandInjection")
