...
def getAuthenticated(user, pwd, urlauth1):
	"""
		Get the content of the url by GET method
	"""
	print "_____INSIDE AUTH-----------"
	resp = ""
	try:
		opener = urllib2.build_opener(urllib2.HTTPCookieProcessor(cj))
		urllib2.install_opener(opener)
		req = Request(urlauth1, None, txheaders) # create a request object
		ret = urlopen(req) 
       ...
       'Origin':'https://129.219.253.30:80',
       'User-Agent': 'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.90 Safari/537.36',
       'Content-Type': 'application/x-www-form-urlencoded',
       'Referer':'https://129.219.253.30:80/',
       'Accept-Encoding': 'gzip, deflate',
       'Accept-Language': 'en-US,en;q=0.8'}
		login_data = urllib.urlencode(payload)
		ret = None
		req = Request(urlauth1, login_data, hdr)  # create a request object
		ret = urlopen(req)
	except HTTPError, e:
		return
	except URLError, e:
...
def getAuthenticatedWithoutCSRF(user, pwd, urlauth1):
	"""
		Get the content of the url by GET method
	"""
	print "_____INSIDE AUTH-----------"
...
def scan(currentURL):
	...
	try:
		...
		print "html content for first scan", htmlContent
	except IOError, e:
